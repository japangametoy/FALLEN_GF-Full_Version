...
(() => {
    ...
    Game_System.prototype.initSpeedStarBattleKe = function() {
        ...
        this._battleWaitSklKe = 0;
        this._battleWaitOptKe = 0;
        // アニメ速度
    ...
    function battleWait() {
        const gs = $gameSystem;
        // 基本値×プラグインコマンド分×早押し分
        let speed = gs._battleWaitSklKe || (gs._battleWaitBscKe - Math.randomInt(gs._battleWaitRdmKe));
    ...
    function processTempTarget(subject, target) {
        ...
        if (!subject._aiRoutinesKe) { return; }
        const bm = BattleManager;
        //  仮バトラーズがなければ作成
    ...
    function getBattleWaitSkill(item, subject, aniSpeed) {
        ...
        $gameSystem._battleWaitSklKe = btwMemo != null ? Number(btwMemo) : 0; 
        if (btwMemo) {
            bm._actionAnimesKe = [];
            return;
        }
    ...
    function reviseBattleWait() {
        ...
        const reviseOn = (keke_battleWaitReviseShort || keke_battleWaitReviseLong) && !$gameSystem._battleWaitSklKe;
    ...
    function updatePopWait(battlerSprite) {
        ...
        battlerSprite._popWaitsKe.forEach((p, i) => {
            ...
            if (p.wait <= 0) {
                // インヴォークアクション(特殊)
                invokeActionEx(p.subject, battlerSprite._battler, p.action);
    ...
    Sprite_Battler.prototype.startMove = function(x, y, duration) {
        // 加速レート
        const fastRate = Math.max(keke_moveSpeed , 0.1);
        // 移動時間を変更
    ...
    Sprite_Actor.prototype.motionSpeed = function() {
        ...
        // 加速レート
        const fastRate = Math.max(keke_motionSpeed, 0.1);
        // 速度変更
